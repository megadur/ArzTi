...
namespace ArzTiServer.Services
{
    public class ArzTiDatenService : IArzTiDatenService
    {
        ...
        private async Task<Rezept> getERez(ErSenderezepteErezept item)
        {
            ...
            ErSenderezepteErezeptDaten erd = item.ErSenderezepteErezeptDatens.First();
            if (erd == null)
                erd = await _datenRepository.GetERezeptDatenAsync(item.IdSenderezepteErezept);

            ErSenderezepteErezeptStatus ers = item.ErSenderezepteErezeptStatuses.First();
            if (ers == null)
                ers = await _datenRepository.GetERezeptStatusAsync(item.IdSenderezepteErezept);

            return new Rezept
            {
                Id = new RezeptId() { Id = item.ErezeptId, Typ = RezeptTyp.ERezeptEnum },
                Uid = new RezeptUId() { Ruid = (erd != null) ? erd.RezeptUuid : null },
                Data = (erd != null) ? erd.XmlRequest : null,
                Status = (ers != null) ? ers.RezeptStatus : null
            };
        ...
        private async Task<RezeptStatus> getERezeptStatus(ErSenderezepteErezept item)
        {
            ...
            if (ers == null)
                ers = await _datenRepository.GetERezeptStatusAsync(item.IdSenderezepteErezept);
            return new RezeptStatus()
            {
                Id = new RezeptId()
                {
                    Id = item.ErezeptId,
                    Typ = RezeptTyp.ERezeptEnum
                },
                Status = (ers != null) ? ers.RezeptStatus : null
            };
        }
        ...
        public async Task<ICollection<Rezept>> GetRezeptIdListAsync(string apoik, RezeptTyp? reztyp, int? maxnum, string zeitraum)
        {
            ...
            if (reztyp == null || reztyp == RezeptTyp.ERezeptEnum)
            {
                ...
                var erl = await _datenRepository.GetERezeptIdListAsync(apoik, maxnum, zeitraum, null);
        ...
        public async Task<ICollection<Rezept>> xGetRezeptIdListByStatusAsync(string apoik, RezeptTyp? reztyp, string zeitraum, IEnumerable<string> status)
        {
        ...
        public async Task<ICollection<Rezept>> GetRezeptIdListByTransferAsync(string apoik, RezeptTyp? reztyp, string zeitraum)
        {
            ...
            if (reztyp == null || reztyp == RezeptTyp.ERezeptEnum)
            {
                var list = await _datenRepository.GetERezeptIdListAsync(apoik, null, zeitraum, null);
        ...
        public async Task<ICollection<RezeptStatus>> GetRezeptIdStatusListAsync(string apoik, RezeptTyp? reztyp, string zeitraum, bool? abgeholt)
        {
            List<RezeptStatus> resList = new();
            if (reztyp == null || reztyp == RezeptTyp.ERezeptEnum)
            {
                var list = await _datenRepository.GetERezeptIdListAsync(apoik,null, zeitraum, abgeholt);
