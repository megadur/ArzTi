...
namespace ArzTiServer.Domain.Repositories
{
    public interface IDatenERepository
    {
        ...
        Task<List<ErSenderezepteErezept>> GetERezeptIdListAsync(string apoik, int? maxnum, string zeitraum, bool? abgeholt);
        Task<List<ErSenderezepteErezept>> GetERezeptIdListByStatusAsync(string apoik, string zeitraum, IEnumerable<string> status);
        Task<List<ErSenderezepteErezept>> xGetERezeptIdListByTransferAsync(string apoik, string zeitraum);
